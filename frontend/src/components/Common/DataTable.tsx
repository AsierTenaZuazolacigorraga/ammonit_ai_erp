...
import { EmptyState, Flex, Table, VStack } from "@chakra-ui/react"
import {
    useQuery,
    type QueryKey,
...
export function DataTable<T, TSearch extends { page: number }>({
    ...
    if (isLoading && !isPlaceholderData) { // Show loading only on first load without placeholder
            ...
            <Table.Root size={{ base: "sm", md: "md" }}>
                ...
                <Table.Body>
                    {items.map((item: T, rowIndex: number) => (
                        <Table.Row
                            key={rowIndex} // Or use item.id if available and type-safe
